...
public:
   ...
   SmartDevice(const string &id, const string &name)
       : logs(new vector<DeviceLog>()) // 初始化日志容器
   {
      totalDevices++;
      this->id = id;
      this->name = name;
   ...
   SmartDevice(const SmartDevice &other)
       : logs(new vector<DeviceLog>(*other.logs)) // 深拷贝日志容器
   { // 深拷贝日志
      totalDevices++;
      id = other.id + "_copy";
      name = other.name;
      ...
      if (isOnline) // 新创建的拷贝如果处于在线状态，需要统计为运行中设备
         runningDevices++;
   ...
   ~SmartDevice()
   {
      ...
      if (isOnline)
      {
         runningDevices--; // 销毁时若仍在线，减少运行中计数
