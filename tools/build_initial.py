...
def make_readme(records: list[dict[str, Any]], manifest: dict[str, Any]) -> str:
    ...
    status_descriptions = {
        ...
        "NOT_IN_CURRENT_PUBLIC_INDEX": "本轮未登录的 YOJ 公开列表未列出；登录态列表可能不同，本站本地归档仍可查看，不等于题面或历史代码失效",
    }
    ...
    lines = [
        ...
        *dashboard_lines,
        ...
        f"- YOJ 公开列表快照：`{online_public_count}` 道，最近发生变化时间 `{online_captured_at}`；每日任务仍会复核未变化的列表，该可见性证据不替代 Accepted 或 `PUBLIC_READY`",
        f"- YOJ 可见性投影：当前公开 `{visibility_counts.get('YOJ_PUBLIC', 0)}` 道；未登录公开列表未列出 `{visibility_counts.get('NOT_IN_CURRENT_PUBLIC_INDEX', 0)}` 道（登录态列表可能不同）；下线只更新可见性，不降级冻结的 `PUBLIC_READY` 内容",
        ...
        "- 自动调度：每天北京时间 22:30–23:30 执行主流程和间隔公开列表复核；23:30–23:55 留作维护前弹性缓冲，不启动新一轮；23:55 起避开站点维护。公开题号发现不依赖账号，若本机登录态或钥匙串不可用，新题仍可只抓题面并记录为 `TOPIC_CAPTURED`，需要账号的源码抓取/在线复验则暂停；未完成请求保留断点，在下一天窗口继续；通过工作树门禁的公开白名单生成物会自动提交并推送到 `main`；YOJ 登录密码只从本机钥匙串注入，不进入 GitHub",
        "- 每日 backlog：在公开列表发现阶段之后，自动筛选所有仍为 `RAW_CAPTURED`、归档中已有 Accepted 完整代码和可提交代码、但尚未完成清洗或在线复验的题目，不受当前 YOJ 公开列表是否仍列出的影响；复用清洗、规范化、本地语法/编译/样例/提交表单门禁，并用 `--reverify-accepted --retry-abnormal` 做在线复核；普通表单和实时部分代码模板均可自动提交，多块/空块模板会按在线框架回收源码；每轮在线提交默认最多 50 道，未完成题目保留 backlog 到下一窗口；只有 `publish_ready.py` 通过源码回收与 `Accepted` 证据后才提升为 `PUBLIC_READY` 并发布，片段独立编译、题目当前不可见、样例或账号状态阻塞的题目继续等待",
