...
def make_readme(records: list[dict[str, Any]], manifest: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        *dashboard_lines,
        ...
        "- 自动调度：每天北京时间 22:30 启动主流程；成功后 launchd 每 15 分钟复核未登录的 YOJ 公开列表，至 23:55 维护窗口前结束；公开题号发现不依赖账号，若本机登录态或钥匙串不可用，新题仍可只抓题面并记录为 `TOPIC_CAPTURED`，需要账号的源码抓取/在线复验则暂停；未完成请求保留断点，在下一天窗口继续；通过工作树门禁的公开白名单生成物会自动提交并推送到 `main`；YOJ 登录密码只从本机钥匙串注入，不进入 GitHub",
