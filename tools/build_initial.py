...
def make_readme(records: list[dict[str, Any]], manifest: dict[str, Any]) -> str:
    ...
    status_descriptions = {
        ...
        "NOT_IN_CURRENT_PUBLIC_INDEX": "本轮原站 YOJ 公开列表未列出；本站本地归档仍可查看，不等于题面或历史代码失效",
    }
    ...
    lines = [
        ...
        *dashboard_lines,
        ...
        f"- YOJ 可见性投影：当前公开 `{visibility_counts.get('YOJ_PUBLIC', 0)}` 道；原站公开列表未列出 `{visibility_counts.get('NOT_IN_CURRENT_PUBLIC_INDEX', 0)}` 道；下线只更新可见性，不降级冻结的 `PUBLIC_READY` 内容",
