#!/usr/bin/env python3
"""Create private C++17/macOS-Clang candidates without changing raw captures.
...
# A deliberately portable, broad C++17 set.  It avoids compiler-internal
# umbrella headers while covering the standard-library facilities commonly
# used by the captured solutions.  Unused headers are harmless and make the
# mechanical normalization conservative; later cleaning may minimize them.
...
def normalize_file(source: Path, target: Path) -> dict[str, Any]:
    ...
    normalized = BITS_INCLUDE.sub(PORTABLE_HEADERS, text)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="在 staging 中生成不依赖 bits/stdc++.h 的 C++17 候选")
    ...
    if not args.check:
        for source in candidates:
            ...
            try:
                report_files.append(normalize_file(source, target))
        ...
        write_json(
            output_root.parent / "cpp17-header-normalization-report.json",
            {
                ...
                "rule": "replace bits/stdc++.h with portable C++17 standard headers",
